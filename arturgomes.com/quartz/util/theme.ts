...
export function googleFontHref(theme: Theme) {
  ...
  // URL-encode font names (spaces become +)
  const encodeFont = (font: string) => font.replace(/ /g, "+")

  return `https://fonts.googleapis.com/css2?family=${encodeFont(headerFont)}&family=${encodeFont(bodyFont)}&family=${encodeFont(codeFont)}&display=swap`
}
...
export function googleFontSubsetHref(theme: Theme, text: string) {
  ...
  return `https://fonts.googleapis.com/css2?family=${encodeFont(titleFont)}&text=${encodeURIComponent(text)}&display=swap`
}
